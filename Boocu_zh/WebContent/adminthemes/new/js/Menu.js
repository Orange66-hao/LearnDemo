...
function onClick(event, treeId, treeNode, clickFlag){
	$("#detail_wraper").load("/admin/menu/edit.jspx?id="+treeNode.menuid,function(){
		//$("#editForm").validate();
...
function deleteMenu(id){
	$.Loading.show('请稍侯...');
	$.ajax({
		url:'/admin/menu/delete.jspx?id='+id,
		type:'post',
		dataType:'json',
		success:function(result){
...
function moveNode(menuid,target_menu_id,moveType){
	...
	$.Loading.show('请稍侯...');
	$.ajax({
		url:"/admin/menu/move.jspx?id="+menuid+"&targetid="+target_menu_id+"&movetype="+moveType,
		type:'post',
		dataType:'json',
		success:function(result){
...
$(function(){
	...
	$("#add-menu-btn").click(function(){
		$("#detail_wraper").load("/admin/menu/add.jspx?parentid=0",function(){
