 ...
 * @version 1.0
 */
@Controller("fastPubWantRentController")
public class FastPubWantRentController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "/front/userCenter/fastPubManage/wantRent";
	 ...
	 * 产品信息求租
	 */
	@RequestMapping(value = { "/fastPubWantRent", "/fastPubWantRent.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userInforSale(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/fastPubWantRent";
		} else {
	...
	// 修改产品信息销售
	@RequestMapping(value = "/edit_fastPubWantRent", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_rentInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_fastWantRent";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改快捷发布销售页面
	@ResponseBody
	@RequestMapping(value = "/save_editFastWantRent.jspx", method = { RequestMethod.POST, RequestMethod.GET })
	public Map<String, Object> save_editFastWantRent(HttpServletRequest request, HttpServletResponse response,
			Model model) {
		long id = ReqUtil.getLong(request, "id", 0l);
		...
		if (productEntity != null) {
			...
			productEntity.getProductWantRentEntity().setRequired(required);
