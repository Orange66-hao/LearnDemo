 ...
 */
@Controller("frontProductWantRentController")
@RequestMapping("/productWantRent")
public class ProductWantRentController {
	
	private static final String TEMPATH = "/template/front/wantRent";
	...
	@Resource
	private ProductRentService proRentService;
	...
	//服务类型ID
	private static final long TYPEID  = Long.parseLong(""+productType.wantRent.getValue());
	
	@RequestMapping(value="/getWantRentNames.jspx",method={RequestMethod.POST,RequestMethod.GET})
	public void getWantRentNames(HttpServletRequest request, HttpServletResponse response, Model model,
		...
		List<ProductWantRentEntity> wants = productWantRentService.getWantRentInfo(params);
		
    	List<Map> resultList = new ArrayList<Map>();
    	for(ProductWantRentEntity item : wants){
	...
	@RequestMapping(value="/wantRentMatchRent.json",method={RequestMethod.POST,RequestMethod.GET})
	public void rentMatchWant(HttpServletRequest request,HttpServletResponse response,Model model){
		...
		Page<ProductRentEntity> rents = null;
		HashMap<String, Object> params = new HashMap<String, Object>();
		
		if("0".equals(keyword)){
			params.put("index", 0);
			//查询所有
			rents = proRentService.getProductRentPage(pageable, params);
		}else{
			//查询求购的品牌和型号
			ProductWantRentEntity pbe = productWantRentService.find(Long.parseLong(keyword));
		...
		List<Map> resultList = new ArrayList<Map>();
		for(ProductRentEntity item : rents.getCont()){
			if(item.getProductEntity() != null){
				Map<String, Object> map = new HashMap<String,Object>();
				map.put("id", item.getId());
		...
		result.put("total",rents.getTotal());
	...
	public String index(@RequestParam(required=false,defaultValue="1") int pageNum,//页码
			...
			@RequestParam(required = false) String serTypeIdste, // 服务类型分类名称
	...
	public String proBuyDetail(
		...
		if(productWantRentEntity != null){
			productWantRentEntity.getProductEntity().setLookTime(productWantRentEntity.getProductEntity().getLookTime()+1);
			productWantRentEntity.getProductEntity().setLookDate(new Date());
			productWantRentService.update(productWantRentEntity);
			
			//保存每天的浏览量
			String sql = "INSERT INTO t_look_val(pro_id,brand_id,pro_no,brand_name,type) VALUES("+productWantRentEntity.getProductEntity().getId()+","+
			productWantRentEntity.getProductEntity().getProductBrandEntity().getId()+",'"+productWantRentEntity.getProductEntity().getProNo()+"','"
			+productWantRentEntity.getProductEntity().getProductBrandEntity().getName()+"',4)";
			lookValService.addLookVal(sql);
			
			//判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			ProductEntity pe = new OrderUtils().DollarToRmb(member,productWantRentEntity.getProductEntity(),sysConfigService);
			if(pe != null){
				productWantRentEntity.setProductEntity(pe);
			}
			model.addAttribute("productWantRent", productWantRentEntity);
		}
		
		//同类热门仪器
		if(productWantRentEntity.getProductEntity().getProductclass() !=null && !productWantRentEntity.getProductEntity().getProductclass().getParentid().equals("01") && !productWantRentEntity.getProductEntity().getProductclass().getMenuid().equals("01")){
			List<Filter> proFilters = new ArrayList<Filter>();
			proFilters.add(Filter.eq("isDel", 0));
			proFilters.add(Filter.eq("productclass", productWantRentEntity.getProductEntity().getProductclass()));	
		...
		//筛选当前商品分类的上级名称
		if(productWantRentEntity.getProductEntity().getProductclass() !=null){
			if(!productWantRentEntity.getProductEntity().getProductclass().getParentid().equals("01") && !productWantRentEntity.getProductEntity().getProductclass().getMenuid().equals("01")){
				ProductclassEntity productclassEntity = productclassService.find(Filter.eq("menuid", productWantRentEntity.getProductEntity().getProductclass().getParentid()));
		...
		//获取所在地
		if(productWantRentEntity.getProductEntity().getAreaProvince()!=null && !productWantRentEntity.getProductEntity().getAreaProvince().equals("")){
			Long provinceId = Long.parseLong(productWantRentEntity.getProductEntity().getAreaProvince());
			...
			if(worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if(productWantRentEntity.getProductEntity().getAreaCity()!=null && !productWantRentEntity.getProductEntity().getAreaCity().equals("")){
					Long cityId = Long.parseLong(productWantRentEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH+"/list_wantRent_details";
	}
	//递归取得商品分类信息
