...
/**
 * Controller -自动化测试方案 add by deng
 * 
 * @author deng
 * @version 1.0 20150119
 */
@Controller("frontProductAutoTestController")
@RequestMapping("/productAutoTest")
public class ProductAutoTestController {

	private static final String TEMPATH = "/template/front/autoTest";
	...
	// 服务类型ID
	private static final long TYPEID = Long.parseLong("" + productType.autoTest.getValue());
	...
	public String proBuyDetail(@PathVariable long id, HttpServletRequest request, HttpServletResponse response,
		...
		if (autoTestEntity != null) {
			autoTestEntity.getProductEntity().setLookTime(autoTestEntity.getProductEntity().getLookTime() + 1);
			autoTestService.update(autoTestEntity);
			
			//判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			ProductEntity pe = new OrderUtils().DollarToRmb(member,autoTestEntity.getProductEntity(),sysConfigService);
			if(pe != null){
				autoTestEntity.setProductEntity(pe);
			}
			model.addAttribute("autoTestEntity", autoTestEntity);
		...
		// 筛选当前商品分类的上级名称
		if (autoTestEntity.getProductEntity().getProductclass() != null) {
			if (!autoTestEntity.getProductEntity().getProductclass().getParentid().equals("01")
					&& !autoTestEntity.getProductEntity().getProductclass().getMenuid().equals("01")) {
				ProductclassEntity productclassEntity = productclassService
						.find(Filter.eq("menuid", autoTestEntity.getProductEntity().getProductclass().getParentid()));
		...
		// 获取所在地
		if (autoTestEntity.getProductEntity().getAreaProvince() != null
				&& !autoTestEntity.getProductEntity().getAreaProvince().equals("")) {
			Long provinceId = Long.parseLong(autoTestEntity.getProductEntity().getAreaProvince());
			...
			if (worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if (autoTestEntity.getProductEntity().getAreaCity() != null
						&& !autoTestEntity.getProductEntity().getAreaCity().equals("")) {
					Long cityId = Long.parseLong(autoTestEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH + "/list_autoTest_details";
	}
