 ...
 * @version 1.0
 */
@Controller("userPubBuyController")
public class UserPubBuyController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/userPub/buy";

	/** 成功信息 */
	private static final Message SUCCESS_MESSAGE = Message.success("操作成功!");

	/** 错误消息 */
	private static final Message ERROR_MESSAGE = Message.error("有效期不能为空!");
	 ...
	 * 客户产品求购
	 */
	@RequestMapping(value = { "/user_pub_buy", "/user_pub_buy.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userPubBuy(HttpServletRequest request, ModelMap model, HttpServletResponse response) {
		// 已登录的会员名
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/user_pub_buy";
		} else {
	 ...
	 * ajax 客户产品求购
	 */
	@RequestMapping(value = "/user_pub_buy/user_pub_buy", method = RequestMethod.POST)
