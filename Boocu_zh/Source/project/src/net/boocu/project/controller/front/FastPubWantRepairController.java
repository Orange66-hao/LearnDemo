 ...
 * Controller - 产品信息快捷求修
 * 
 * @author deng 20151228
 * @version 1.0
 */
@Controller("fastPubWantRepairController")
public class FastPubWantRepairController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/fastPubManage/wantRepair";
	 ...
	 * 产品信息求修
	 */
	@RequestMapping(value = { "/fastPubWantRepair", "/fastPubWantRepair.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userInforSale(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/fastPubWantRepair";
		} else {
	...
	// 修改产品信息求修
	@RequestMapping(value = "/edit_fastPubWantRepair", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_rentInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_fastWantRepair";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改快捷发布求修修页面
	@ResponseBody
	@RequestMapping(value = "/save_editFastWantRepair.jspx", method = { RequestMethod.POST, RequestMethod.GET })
