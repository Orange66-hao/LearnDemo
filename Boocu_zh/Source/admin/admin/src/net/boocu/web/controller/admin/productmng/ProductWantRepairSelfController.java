 ...
 * 2015年8月31日
 */
@Controller("productSelfWantRepairController")
@RequestMapping("/admin/prodoctMng/selfProMng/wantRepair")
public class ProductWantRepairSelfController {
	
	private static final String TEM_PATH ="/template/admin/prodoctMng/selfProMng/wantRepair";
	...
	//跳转到村实体添加页面
    @RequestMapping(value = "/add_selfPubWantRepair.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    /**
     * 
     * 方法:将新添加的ProductWantRepairEntity（本身实体 ）保存到数据库中
     * 传入参数:ProductWantRepairEntity的字段
     * 传出参数:result（方法结果信息）
     * 逻辑：接收前台ProductWantRepairEntity的字段，对非空字段校验，新建实体赋值保存
     */
    @ResponseBody
    @RequestMapping(value = "/save_selfPubWantRepair.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    /**
     * 
     * 方法:删除选中ProductWantRepairEntity（广告本身实体 ）并更新数据库
     * 传入参数:id（选中实体的id数组）
     * 传出参数:result（方法结果信息）
     */
    @RequestMapping(value = "/delete_selfPubWantRepair.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    //跳转到实体更新页面
    @RequestMapping(value = "/edit_selfPubWantRepair.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    /**
     * 
     * 方法:保存更新之后的ProductWantRepairEntity（村本身实体 ）
     * 传入参数:ProductWantRepairtEntity的字段    id（更新实体的id）
     * 传出参数:result（方法结果信息）
     * 逻辑：查询该id的实体，存在则读取前台ProductWantRepairEntity（村本身实体 ）的字段并执行更新操作；否则提示更新错误
     */
    @ResponseBody
    @RequestMapping(value = "/save_edit_selfPubWantRepair.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    public Map<String,Object> saveEditProductWantRepair(HttpServletRequest request, HttpServletResponse response, Model model) {
    	long id = ReqUtil.getLong(request, "id", 0l);
    	//获取到页面的基本信息
    	long brandId= ReqUtil.getLong(request, "brandId",0l);
    	String proNo = ReqUtil.getString(request, "proNo", "");
    	...
    	if(productWantRepairEntity != null){
        	...
        	productWantRepairEntity.getProductEntity().setProStock(proStock);
