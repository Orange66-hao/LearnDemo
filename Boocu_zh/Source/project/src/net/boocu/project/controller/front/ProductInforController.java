 ...
 * @version 1.0
 */
@Controller("ProductInforController")
public class ProductInforController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/productInforManage";
	 ...
	 * 产品信息管理
	 */
	@RequestMapping(value = { "/productInfor", "/productInfor.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userInforSale(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		return TEMPLATE_PATH + "/productInfor";
	}

	// 修改产品信息
	@RequestMapping(value = "/edit_productInfor", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_productInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		/*
		 * if(productEntity.getProClass() ==3){
		 * if(productEntity.getProductType().getId() ==1){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/sale/edit_fastSale"; }else
		 * if(productEntity.getProductType().getId() ==2){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/buy/edit_fastBuy"; }else
		 * if(productEntity.getProductType().getId() ==3){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/rent/edit_fastRent"; }else
		 * if(productEntity.getProductType().getId() ==4){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/wantRent/edit_fastWantRent";
		 * }else if(productEntity.getProductType().getId() ==5){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/repair/edit_fastRepair"; }else
		 * if(productEntity.getProductType().getId() ==6){
		 * model.addAttribute("item", productEntity); return
		 * "/front/userCenter/productInforManage/wantRepair/edit_fastWantRepair";
		 * } }else
		 */
		if (productEntity.getProductType().getId() == 1) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/sale/edit_user_pub_sale";
		} else if (productEntity.getProductType().getId() == 2) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/buy/edit_user_pub_buy";
		} else if (productEntity.getProductType().getId() == 3) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/rent/edit_user_pub_rent";
		} else if (productEntity.getProductType().getId() == 4) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/wantRent/edit_user_pub_wantRent";
		} else if (productEntity.getProductType().getId() == 5) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/repair/edit_user_pub_repair";
		} else if (productEntity.getProductType().getId() == 6) {
			model.addAttribute("item", productEntity);
			return "/front/userCenter/productInforManage/wantRepair/edit_user_pub_wantRepair";
		} else {
			return null;
		}
	}
