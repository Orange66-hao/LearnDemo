 ...
 * @version 1.0
 */
@Controller("repairInforController")
public class RepairInforController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/productInforManage/repair";
	 ...
	 * 产品信息维修
	 */
	@RequestMapping(value = { "/repairInfor", "/repairInfor.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userInforSale(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			pageable4.getFilters().add(Filter.eq("productType", productTypeService.find(5l)));
			...
			return TEMPLATE_PATH + "/repairInfor";
		} else {
	...
	// 修改产品信息维修
	@RequestMapping(value = "/edit_repairInfor", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_rentInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_user_pub_repair";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改客户商品销售
	@ResponseBody
	@RequestMapping(value = "/save_updateEdit_user_pub_repair", method = { RequestMethod.POST, RequestMethod.GET })
