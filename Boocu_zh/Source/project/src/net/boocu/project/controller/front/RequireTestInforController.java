 ...
 * @version 1.0
 */
@Controller("requireTestInforController")
public class RequireTestInforController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/productInforManage/requireTest";
	...
	@ModelAttribute
	public void getRequireTestInfor(@RequestParam(required = false, defaultValue = "0") Long id, Model model) {
	...
	/**
	 */
	@RequestMapping(value = { "/requireTestInfor", "/requireTestInfor.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userRequireTestInfor(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			pageable.getFilters().add(Filter.eq("productType", productTypeService.find(12l)));
			...
			pageable1.getFilters().add(Filter.eq("productType", productTypeService.find(12l)));
			...
			pageable2.getFilters().add(Filter.eq("productType", productTypeService.find(12l)));
			...
			pageable3.getFilters().add(Filter.eq("productType", productTypeService.find(12l)));
			...
			pageable4.getFilters().add(Filter.eq("productType", productTypeService.find(12l)));
			...
			return TEMPLATE_PATH + "/requireTestInfor";
		} else {
	...
	// 修改产品信息销售
	@RequestMapping(value = "/edit_requireTestInfor", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_requireTestInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_user_pub_requireTest";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改客户商品销售
	@ResponseBody
	@RequestMapping(value = "/save_updateEdit_user_pub_requireTest", method = { RequestMethod.POST, RequestMethod.GET })
	public Message save_updateEdit_user_pub_requireTest(HttpServletRequest request, HttpServletResponse response,
			String[] brandName, @ModelAttribute("editRequireTest") RequireTestEntity requireTestEntity,
			InstrumentsBean instrumentsBean) {
		if (requireTestEntity.getProductEntity().getInforValidity() == null
				&& requireTestEntity.getProductEntity().getLongTerm() == 0) {
			return ERROR_MESSAGE;
		}

		List<InstrumentEntity> list = requireTestEntity.getProductItems();
		requireTestEntity.getProductEntity().setApprStatus(0);
