 ...
 * @version 1.0
 */
@Controller("productTestInforController")
public class ProductTestInforController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/productInforManage/productTest";
	...
	/**
	 */
	@RequestMapping(value = { "/productTestInfor", "/productTestInfor.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userAutoTestInfor(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			pageable.getFilters().add(Filter.eq("productType", productTypeService.find(11l)));
			...
			pageable1.getFilters().add(Filter.eq("productType", productTypeService.find(11l)));
			...
			pageable2.getFilters().add(Filter.eq("productType", productTypeService.find(11l)));
			...
			pageable3.getFilters().add(Filter.eq("productType", productTypeService.find(11l)));
			...
			pageable4.getFilters().add(Filter.eq("productType", productTypeService.find(11l)));
			...
			return TEMPLATE_PATH + "/productTestInfor";
		} else {
	...
	// 修改产品信息销售
	@RequestMapping(value = "/edit_productTestInfor", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_autoTestInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_user_pub_productTest";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改客户商品销售
	@ResponseBody
	@RequestMapping(value = "/save_updateEdit_user_pub_productTest", method = { RequestMethod.POST, RequestMethod.GET })
	public Message save_updateEdit_user_pub_productTest(HttpServletRequest request, HttpServletResponse response,
			String[] brandName, @ModelAttribute("editProductTest") ProductTestEntity productTestEntity,
			InstrumentsBean instrumentsBean) {
		if (productTestEntity.getProductEntity().getInforValidity() == null
				&& productTestEntity.getProductEntity().getLongTerm() == 0) {
			return ERROR_MESSAGE;
		}
		List<InstrumentEntity> list = productTestEntity.getProductItems();
		productTestEntity.getProductEntity().setApprStatus(0);
