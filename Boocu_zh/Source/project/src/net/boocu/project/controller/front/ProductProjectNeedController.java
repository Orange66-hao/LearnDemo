...
/**
 * Controller -自动化测试方案需求 add by deng
 * 
 * @author deng
 * @version 1.0 20150127
 */
@Controller("frontProductProjectNeedController")
@RequestMapping("/productProjectNeed")
public class ProductProjectNeedController {

	private static final String TEMPATH = "/template/front/projectNeed";
	...
	// 服务类型ID
	private static final long TYPEID = Long.parseLong("" + productType.projectNeed.getValue());
	...
	public String index(@RequestParam(required = false, defaultValue = "1") int pageNum, // 页码
		...
		return TEMPATH + "/list_projectNeed";
	}
	...
	public String proBuyDetail(@PathVariable long id, HttpServletRequest request, HttpServletResponse response,
		...
		if (projectNeedEntity != null) {
			projectNeedEntity.getProductEntity().setLookTime(projectNeedEntity.getProductEntity().getLookTime() + 1);
			projectNeedService.update(projectNeedEntity);
			
			//判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			ProductEntity pe = new OrderUtils().DollarToRmb(member,projectNeedEntity.getProductEntity(),sysConfigService);
			if(pe != null){
				projectNeedEntity.setProductEntity(pe);
			}
			model.addAttribute("projectNeedEntity", projectNeedEntity);
		...
		// 筛选当前商品分类的上级名称
		if (projectNeedEntity.getProductEntity().getProductclass() != null) {
			if (!projectNeedEntity.getProductEntity().getProductclass().getParentid().equals("01")
					&& !projectNeedEntity.getProductEntity().getProductclass().getMenuid().equals("01")) {
				ProductclassEntity productclassEntity = productclassService.find(
						Filter.eq("menuid", projectNeedEntity.getProductEntity().getProductclass().getParentid()));
		...
		// 获取所在地
		if (projectNeedEntity.getProductEntity().getAreaProvince() != null
				&& !projectNeedEntity.getProductEntity().getAreaProvince().equals("")) {
			Long provinceId = Long.parseLong(projectNeedEntity.getProductEntity().getAreaProvince());
			...
			if (worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if (projectNeedEntity.getProductEntity().getAreaCity() != null
						&& !projectNeedEntity.getProductEntity().getAreaCity().equals("")) {
					Long cityId = Long.parseLong(projectNeedEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH + "/list_projectNeed_details";
	}
