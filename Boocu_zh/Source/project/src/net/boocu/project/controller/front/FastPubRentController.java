 ...
 * @version 1.0
 */
@Controller("fastPubRentController")
public class FastPubRentController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/fastPubManage/rent";
	 ...
	 * 产品信息求购
	 */
	@RequestMapping(value = { "/fastPubRent", "/fastPubRent.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String fastPubRent(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/fastPubRent";
		} else {
	...
	// 修改产品信息销售
	@RequestMapping(value = "/edit_fastPubRent", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_rentInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_fastRent";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改快捷发布销售页面
	@ResponseBody
	@RequestMapping(value = "/save_editFastRent.jspx", method = { RequestMethod.POST, RequestMethod.GET })
	public Map<String, Object> save_editFastRent(HttpServletRequest request, HttpServletResponse response,
			Model model) {
		long id = ReqUtil.getLong(request, "id", 0l);
