 ...
 * 2015年10月28日
 */
@Controller("autoTestCusController")
@RequestMapping("/admin/prodoctMng/cusProMng/autoTest")
public class AutoTestCusController {
	
	private static final String TEM_PATH ="/template/admin/prodoctMng/cusProMng/autoTest";
	...
	//跳转到村实体添加页面
    @RequestMapping(value = "/add_cusPubAutoTest.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/save_cusPubAutoTest.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    public Map<String,Object> saveAutoTest(HttpServletRequest request, HttpServletResponse response, Model model) {
    	...
    	RateEnum rateEnum = taxRate.isEmpty()?null: RateEnum.valueOf(taxRate);
    	productEntity.setTaxRate(rateEnum);
     ...
     * 传出参数:result（方法结果信息）
     */
    @RequestMapping(value = "/delete_cusPubAutoTest.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    //跳转到实体更新页面
    @RequestMapping(value = "/edit_cusPubAutoTest.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/save_edit_cusPubAutoTest.jspx",method ={ RequestMethod.POST, RequestMethod.GET})
    ...
    /**显示仪器列表*/
    @RequestMapping(value="/itemData.json",method={RequestMethod.POST,RequestMethod.GET})
	public void itemData(HttpServletRequest request,HttpServletResponse response,Model model){
		int pagenumber = ReqUtil.getInt(request, "page", 0);
		int rows = ReqUtil.getInt(request, "rows", 10);
		String sortValue = ReqUtil.getString(request, "sort", "");
		String sortOrder = ReqUtil.getString(request, "order", "desc");
		Long testId = ReqUtil.getLong(request, "id", 0l);
		Pageable pageable = new Pageable(pagenumber,rows);
		if(!sortValue.isEmpty()){
			sortValue = sortValue.replace("item", "instrumentEntity");
			List<Sequencer> ilSequencers = new ArrayList<Sequencer>();
			Sequencer sequencer = "desc".equals(sortOrder) ? Sequencer.desc(sortValue) :Sequencer.asc(sortValue) ;
			ilSequencers.add(sequencer);
			pageable.setSequencers(ilSequencers);
		...
		if(testId != 0l){
			AutoTestEntity autoTestEntity = autoTestService.find(testId);
			if(autoTestEntity!= null){
				pageable.getFilters().add(Filter.eq("autoTestEntity",autoTestEntity ));
			}
		}
		Page<InstrumentEntity> page = instrumentService.findPage(pageable);
		List<Map> resultList = new ArrayList<Map>();
		for(InstrumentEntity item : page.getCont()){
			Map<String, Object> map = new HashMap<String,Object>();
			if(item != null){
				JsonConfig jsonConfig = new JsonConfig();
				jsonConfig.setJsonPropertyFilter(new PropertyFilter() {
    ...
    /**至增加仪器界面*/
    @RequestMapping(value="/addItem.jspx")
    public String toAddItem(){
    	return TEM_PATH+"/instru_add";
    }
    /**至修改仪器界面*/
    @RequestMapping(value="/editItem.jspx")
    public String toEditItem(Long id,Model model){
    	if(id != null){
    		model.addAttribute("item", instrumentService.find(id));
    	}
    	return TEM_PATH+"/instru_edit";
    }
    /**保存增加仪器*/
    @ResponseBody
    @RequestMapping(value="/saveOrUpdItem.jspx")
    public Map<String,Object> saveAddItem(InstrumentEntity instrumentEntity){
    	ProductclassEntity productclassEntity = productclassService.find(Filter.eq("menuid",  instrumentEntity.getProductclass().getId()));
    	if(productclassEntity != null){
    		instrumentEntity.setProductclass(productclassEntity);
    	}
    	if(instrumentEntity.getId() != null){
    		instrumentService.update(instrumentEntity);
    	}else{
    		instrumentService.save(instrumentEntity);
    	}
    	Map<String,Object> result = new HashMap<String, Object>();
    	result.put("result", 1);
    	result.put("message", "操作成功");
    ...
    /**删除仪器*/
    @RequestMapping(value="/delItem.jspx")
    public Map<String,Object> delItem(Long []ids){
    	instrumentService.deleteList(ids);
    	Map<String,Object> result = new HashMap<String, Object>();
    	result.put("result", 1);
    	result.put("message", "操作成功");
    	return result;
