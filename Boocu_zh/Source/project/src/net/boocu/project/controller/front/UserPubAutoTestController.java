 ...
 * @version 1.0
 */
@Controller("userPubautoTestController")
public class UserPubAutoTestController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/userPub/autoTest";
	 ...
	 * 自动化测试需求
	 */
	@RequestMapping(value = { "/user_pub_autoTest", "/user_pub_autoTest.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userPubautoTest(HttpServletRequest request, ModelMap model, HttpServletResponse response) {
		// 已登录的会员名
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/user_pub_autoTest";
		} else {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/user_pub_autoTest/user_pub_autoTest", method = { RequestMethod.POST, RequestMethod.GET })
	public Message userPubRent(HttpServletRequest request, HttpServletResponse response, AutoTestEntity autoTestEntity,
			String[] brandName) {
		...
		// 判断是发布类型todo
		autoTestService.saveWithPro(autoTestEntity, brandName);
