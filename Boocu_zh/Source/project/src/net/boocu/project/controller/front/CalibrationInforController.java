 ...
 * @version 1.0
 */
@Controller("calibrationInforController")
public class CalibrationInforController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/productInforManage/calibration";
	...
	@ModelAttribute
	public void getCalibrationInfor(@RequestParam(required = false, defaultValue = "0") Long id, Model model) {
	...
	/**
	 */
	@RequestMapping(value = { "/calibrationInfor", "/calibrationInfor.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String userRequireTestInfor(@RequestParam(required = false, defaultValue = "1") int pageNum, // 审核页码
		...
		if (currentMember != null) {
			...
			pageable.getFilters().add(Filter.eq("productType", productTypeService.find(13l)));
			...
			pageable1.getFilters().add(Filter.eq("productType", productTypeService.find(13l)));
			...
			pageable2.getFilters().add(Filter.eq("productType", productTypeService.find(13l)));
			...
			pageable3.getFilters().add(Filter.eq("productType", productTypeService.find(13l)));
			...
			pageable4.getFilters().add(Filter.eq("productType", productTypeService.find(13l)));
			...
			return TEMPLATE_PATH + "/calibrationInfor";
		} else {
	...
	// 修改产品信息销售
	@RequestMapping(value = "/edit_calibrationInfor", method = { RequestMethod.GET, RequestMethod.POST })
	public String edit_calibrationInfor(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/edit_user_pub_calibration";
		} else {
			return "redirect:/login.jhtml";
		}
	}
	...
	// 修改客户商品销售
	@ResponseBody
	@RequestMapping(value = "/save_updateEdit_user_pub_calibration", method = { RequestMethod.POST, RequestMethod.GET })
	public Message save_updateEdit_user_pub_calibration(HttpServletRequest request, HttpServletResponse response,
			String[] brandName, String brand, @ModelAttribute("editCalibration") CalibrationEntity calibrationEntity,
		...
		calibrationEntity.getProductEntity().setApprStatus(0);
		if (brand != null) {
			if (!brand.isEmpty()) {
