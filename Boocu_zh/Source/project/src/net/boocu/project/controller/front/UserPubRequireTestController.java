 ...
 * @version 1.0
 */
@Controller("userPubRequireTestController")
public class UserPubRequireTestController extends BaseController {
	/** 模板路径 */
	private static final String TEMPLATE_PATH = "front/userCenter/userPub/requireTest";
	 ...
	 * 客户产品测试
	 */
	@RequestMapping(value = { "/user_pub_requireTest", "/user_pub_requireTest.jhtml" }, method = { RequestMethod.GET,
			RequestMethod.POST })
	public String user_pub_requireTest(HttpServletRequest request, ModelMap model, HttpServletResponse response) {
		// 已登录的会员名
		...
		if (currentMember != null) {
			...
			return TEMPLATE_PATH + "/user_pub_requireTest";
		} else {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/user_pub_requireTest/user_pub_requireTest", method = { RequestMethod.POST,
			RequestMethod.GET })
	public Message user_pub_requireTests(HttpServletRequest request, HttpServletResponse response,
			RequireTestEntity requireTestEntity, String[] brandName) {
		...
		// 判断是发布类型todo
		requireTestService.saveWithRequireTest(requireTestEntity, brandName);
