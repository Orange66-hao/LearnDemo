 ...
 */
@Controller("frontProductRequireTestController")
@RequestMapping("/productRequireTest")
public class ProductRequireTestController {

	private static final String TEMPATH = "/template/front/requireTest";
	...
	// 服务类型ID
	private static final long TYPEID = Long.parseLong("" + productType.requireTest.getValue());
	...
	public String index(@RequestParam(required = false, defaultValue = "1") int pageNum, // 页码
		...
		return TEMPATH + "/list_requireTest";
	}
	...
	public String proBuyDetail(@PathVariable long id, HttpServletRequest request, HttpServletResponse response,
		...
		if (requireTestEntity != null) {
			requireTestEntity.getProductEntity().setLookTime(requireTestEntity.getProductEntity().getLookTime() + 1);
			requireTestService.update(requireTestEntity);
			
			//判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			ProductEntity pe = new OrderUtils().DollarToRmb(member,requireTestEntity.getProductEntity(),sysConfigService);
			if(pe != null){
				requireTestEntity.setProductEntity(pe);
			}
			model.addAttribute("requireTestEntity", requireTestEntity);
		...
		// 筛选当前商品分类的上级名称
		if (requireTestEntity.getProductEntity().getProductclass() != null) {
			if (!requireTestEntity.getProductEntity().getProductclass().getParentid().equals("01")
					&& !requireTestEntity.getProductEntity().getProductclass().getMenuid().equals("01")) {
				ProductclassEntity productclassEntity = productclassService.find(
						Filter.eq("menuid", requireTestEntity.getProductEntity().getProductclass().getParentid()));
		...
		// 获取所在地
		if (requireTestEntity.getProductEntity().getAreaProvince() != null
				&& !requireTestEntity.getProductEntity().getAreaProvince().equals("")) {
			Long provinceId = Long.parseLong(requireTestEntity.getProductEntity().getAreaProvince());
			...
			if (worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if (requireTestEntity.getProductEntity().getAreaCity() != null
						&& !requireTestEntity.getProductEntity().getAreaCity().equals("")) {
					Long cityId = Long.parseLong(requireTestEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH + "/list_requireTest_details";
	}
