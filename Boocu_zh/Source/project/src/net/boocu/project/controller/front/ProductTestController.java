 ...
 */
@Controller("frontProductTestController")
@RequestMapping("/productTest")
public class ProductTestController {

	private static final String TEMPATH = "/template/front/productTest";
	...
	// 服务类型ID
	private static final long TYPEID = Long.parseLong("" + productType.productTest.getValue());
	...
	public String index(@RequestParam(required = false, defaultValue = "1") int pageNum, // 页码
		...
		return TEMPATH + "/list_productTest";
	}
	...
	public String proBuyDetail(@PathVariable long id, HttpServletRequest request, HttpServletResponse response,
		...
		if (productTestEntity != null) {
			productTestEntity.getProductEntity().setLookTime(productTestEntity.getProductEntity().getLookTime() + 1);
			productTestService.update(productTestEntity);
			
			//判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			ProductEntity pe = new OrderUtils().DollarToRmb(member,productTestEntity.getProductEntity(),sysConfigService);
			if(pe != null){
				productTestEntity.setProductEntity(pe);
			}
			model.addAttribute("productTestEntity", productTestEntity);
		...
		// 筛选当前商品分类的上级名称
		if (productTestEntity.getProductEntity().getProductclass() != null) {
			if (!productTestEntity.getProductEntity().getProductclass().getParentid().equals("01")
					&& !productTestEntity.getProductEntity().getProductclass().getMenuid().equals("01")) {
				ProductclassEntity productclassEntity = productclassService.find(
						Filter.eq("menuid", productTestEntity.getProductEntity().getProductclass().getParentid()));
		...
		// 获取所在地
		if (productTestEntity.getProductEntity().getAreaProvince() != null
				&& !productTestEntity.getProductEntity().getAreaProvince().equals("")) {
			Long provinceId = Long.parseLong(productTestEntity.getProductEntity().getAreaProvince());
			...
			if (worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if (productTestEntity.getProductEntity().getAreaCity() != null
						&& !productTestEntity.getProductEntity().getAreaCity().equals("")) {
					Long cityId = Long.parseLong(productTestEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH + "/list_productTest_details";
	}
