 ...
 * 
 * @author deng
 * @version 1.0 20150921
 */
@Controller("frontProductWantRepairController")
@RequestMapping("/productWantRepair")
public class ProductWantRepairController {

	private static final String TEMPATH = "/template/front/wantRepair";
	...
	@Resource
	private ProductRepairService proRepairService;
	...
	// 服务类型ID
	private static final long TYPEID = Long.parseLong("" + productType.wantRepair.getValue());

	@RequestMapping(value = "/getWantRepairNames.jspx", method = { RequestMethod.POST, RequestMethod.GET })
	public void getWantRepairNames(HttpServletRequest request, HttpServletResponse response, Model model,
		...
		List<ProducWantRepairEntity> wantRepairs = productWantRepairService.getWantRepair(params);

		List<Map> resultList = new ArrayList<Map>();
		for (ProducWantRepairEntity item : wantRepairs) {
	...
	@RequestMapping(value = "wantRepairMatchRepair.json", method = { RequestMethod.POST, RequestMethod.GET })
	public void repairMatchWantRepair(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		Page<ProductRepairEntity> repairs = null;
		HashMap<String, Object> params = new HashMap<String, Object>();
		...
		if ("0".equals(keyword)) {
			params.put("index", 0);
			// 查询所有
			repairs = proRepairService.getProductRepairPage(pageable, params);
		} else {
			// 查询求购的品牌和型号
			// ProductRepairEntity pbe =
			// proRepairService.find(Long.parseLong(keyword));
			ProducWantRepairEntity pbe = productWantRepairService.find(Long.parseLong(keyword));
		...
		List<Map> resultList = new ArrayList<Map>();
		for (ProductRepairEntity item : repairs.getCont()) {
			if (item.getProductEntity() != null) {
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("id", item.getId());
				map.put("name",
		...
		result.put("total", repairs.getTotal());
	...
	public String index(@RequestParam(required = false, defaultValue = "1") int pageNum, // 页码
		...
		return TEMPATH + "/list_wantRepair";
	}
	...
	public String proBuyDetail(@PathVariable long id, HttpServletRequest request, HttpServletResponse response,
		...
		if (productWantRepairEntity != null) {
			productWantRepairEntity.getProductEntity()
					.setLookTime(productWantRepairEntity.getProductEntity().getLookTime() + 1);
			productWantRepairEntity.getProductEntity().setLookDate(new Date());
			productWantRepairService.update(productWantRepairEntity);

			// 保存每天的浏览量
			String sql = "INSERT INTO t_look_val(pro_id,brand_id,pro_no,brand_name,type) VALUES("
					+ productWantRepairEntity.getProductEntity().getId() + ","
					+ productWantRepairEntity.getProductEntity().getProductBrandEntity().getId() + ",'"
					+ productWantRepairEntity.getProductEntity().getProNo() + "','"
					+ productWantRepairEntity.getProductEntity().getProductBrandEntity().getName() + "',6)";
			lookValService.addLookVal(sql);

			// 判断请求用户是否登录
			MemberEntity member = (MemberEntity) request.getSession().getAttribute("loginUser");
			if (member != null) {
				ProductEntity pe = new OrderUtils().DollarToRmb(member, productWantRepairEntity.getProductEntity(),
						sysConfigService);
				if (pe != null) {
					productWantRepairEntity.setProductEntity(pe);
				}
			}
			model.addAttribute("productWantRepair", productWantRepairEntity);
		}

		// 同类热门仪器
		if (productWantRepairEntity.getProductEntity().getProductclass() != null
				&& !productWantRepairEntity.getProductEntity().getProductclass().getParentid().equals("01")
				&& !productWantRepairEntity.getProductEntity().getProductclass().getMenuid().equals("01")) {
			List<Filter> proFilters = new ArrayList<Filter>();
			proFilters.add(Filter.eq("isDel", 0));
			proFilters.add(Filter.eq("productclass", productWantRepairEntity.getProductEntity().getProductclass()));
		...
		// 筛选当前商品分类的上级名称
		if (productWantRepairEntity.getProductEntity().getProductclass() != null
				&& !productWantRepairEntity.getProductEntity().getProductclass().equals("")) {
			if (!productWantRepairEntity.getProductEntity().getProductclass().getParentid().equals("01")
					&& !productWantRepairEntity.getProductEntity().getProductclass().getParentid().equals("01")) {
				ProductclassEntity productclassEntity = productclassService.find(Filter.eq("menuid",
						productWantRepairEntity.getProductEntity().getProductclass().getParentid()));
				model.addAttribute("productClass2", productclassEntity.getName());
				...
				if (productclassEntity2 != null && !productclassEntity2.getMenuid().equals("01")
					...
					model.addAttribute("productClass1", productclassEntity2.getName());
				}
			}
		}
		// 获取所在地
		if (productWantRepairEntity.getProductEntity().getAreaProvince() != null
				&& !productWantRepairEntity.getProductEntity().getAreaProvince().equals("")) {
			Long provinceId = Long.parseLong(productWantRepairEntity.getProductEntity().getAreaProvince());
			...
			if (worldAreaEntity.getAREANAME().equals("天津市") || worldAreaEntity.getAREANAME().equals("北京市")
				...
				if (productWantRepairEntity.getProductEntity().getAreaCity() != null
						&& !productWantRepairEntity.getProductEntity().getAreaCity().equals("")) {
					Long cityId = Long.parseLong(productWantRepairEntity.getProductEntity().getAreaCity());
		...
		return TEMPATH + "/list_wantRepair_details";
	}
